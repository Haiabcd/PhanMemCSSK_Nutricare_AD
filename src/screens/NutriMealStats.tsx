    ...
    <div className="h-64 flex items-end gap-5">
      {bins.map((b) => {
        ...
        return (
          <div
            key={b.label}
            className="h-full flex-1 flex flex-col items-center justify-end"
          >
            <div
...
export default function NutriMealStats() {
  ...
  useEffect(() => {
    ...
    let mounted = true; // guard tránh setState sau unmount
    setLoading(true);
    ...
    fetchOverviewNutrition(ac.signal)
      ...
      .catch((err: unknown) => {
        ...
        console.error("Lỗi tải thống kê dinh dưỡng:", toAxiosMessage(err));
        if (!mounted || ac.signal.aborted) return;
        setData(null);
      })
      .finally(() => {
      ...
      <div className="space-y-5">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Tổng quan về chất lượng dữ liệu dinh dưỡng của các món ăn trong hệ
            thống.
          </p>
        </div>

        <div
          className={`grid sm:grid-cols-3 xl:grid-cols-4 gap-5 ${
        ...
        >
          ...
          <StatCard
            ...
            value={
              data ? `${data.countFoodsWithComplete5}/${data.totalFoods}` : "—"
            }
